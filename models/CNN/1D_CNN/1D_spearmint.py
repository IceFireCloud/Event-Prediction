...
settings['train_end'] = 2567
#settings['drop_zeros'] = True
#settings['integrate_signals'] = True

if spearmint:
    # take command line input and give it proper names
    # note the list slicing - for some reason we're fed (e.g.) batch sizes in the form '[64]'
    job_id = int(sys.argv[1])
    n_epochs = int(sys.argv[2][1:-1])
    l_r = float(sys.argv[3][1:-1])
    dropout_rate = float(sys.argv[4][1:-1])
    num_filters = int(sys.argv[5][1:-1])
    conv_layers = int(sys.argv[6][1:-1])
    first_pool = int(sys.argv[7][1:-1])
    second_pool = int(sys.argv[8][1:-1])
    l1_reg = float(sys.argv[9][1:-1])
    train_end = int(sys.argv[10][1:-1])

    settings['n_epoch'] = n_epochs
    settings['l_r'] = l_r
    settings['dropout_rate'] = dropout_rate
    settings['num_filters'] = num_filters
    settings['train_end'] = train_end
    settings['conv_layers'] = conv_layers
    settings['first_pool'] = first_pool
    settings['second_pool'] = second_pool
    settings['l1_reg'] = l1_reg
    settings['train_end'] = train_end
...
def one_grid(block_num, save_path):
    ...
    print('testing rmse from keras:  ', np.sqrt(score[1]))

    #for spearmint propose only
    print('$' + str(np.sqrt(score[1])) + '$$')
    ...
    filename_s = save_path + '/block_' + str(block_num) + '.csv'
