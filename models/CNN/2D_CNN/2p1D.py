#1 pre-trained network that predict some time later
#2 output of the networks get combined -> another network that combine these info
...
def pre_data(block_num = 0, offset = 0):
    #2401*168*112 => 2401*168*256 => 2401*16*16*168 /=> 2401*16*16*1
    # train and test data
    ...
    while label_pointer <= train_end:
        ...
        train_y_piece = label[label_pointer+predict_ahead+offset]
    ...
    while label_pointer <= predict_end:
        ...
        test_y_piece = label[label_pointer+predict_ahead+offset]
...
#create a network and train it, then return the trained model
def build_train(block_num = 0, offset = 0):
    """Return sum of squared errors for a single grid block"""
    train_x, train_y, test_x, test_y = pre_data(block_num, offset)
    ...
    #filename_s = '../../results/2p1d/initial_discretized_2_' + str(block_num) + '.csv'
    #results.to_csv(filename_s)
    squared_error = np.sum( (test_y - prediction)**2)
...
#create the dataset from trained network, then train it use this data
def final_train():
